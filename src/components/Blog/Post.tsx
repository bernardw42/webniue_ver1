import img from "@/../public/blog/posts/arts.webp";

export const posts = [
    {
      id: 1,
      title: 'DIGITAL CONSULTANCY HARUS HSDGU YES',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a measurable Return on Investment (ROI) asdaksjhd alshdajlkshd ajlkshdjahsdlk alsdjhajlkshd alkshdlahsdka damn lmao asikyudiasytd aosyduays asiduyauisy yausydyuas yes lmai.",
      image: img,
      links: "/article/pembelian-jasa"
    },
    {
      id: 2,
      title: 'CONVERSION RATE OPTIMIZATION',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our client that result in defining prioritized cost-effective solutions that deliver a measurable Return on Investment",
      image: img,
      links: "/article/pembelian-jasa"
    },
    {
      id: 3,
      title: 'ONLINE REPUTATION MANAGEMENT SAKABLUE YEAH',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a akdbaksbdlksbgfjhlkbfgks skdfhlksj dhfjlksdhflkjsdhfuliwehwuoih wfh dhf lksdhf lkehiwuh fcdnhflk whdfliwho; hfnoidhgo; hnghsuoi ghipeuhgne;ng gjhowejgo;sjhgrhgegj erogjhoiejhg9fh.",
      image: img,
      links: "/article/pembelian-jasa"
    },
    {
      id: 4,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/article/pembelian-jasa"
    },
    {
      id: 5,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/article/pembelian-jasa"
    },
    {
      id: 6,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 7,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 8,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 9,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 10,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 11,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 12,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 13,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 14,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 15,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 16,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 17,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 18,
      title: 'CONVERSION RATE OPTIMIZATION',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our client that result in defining prioritized cost-effective solutions that deliver a measurable Return on Investment",
      image: img,
      links: "/"
    },
    {
      id: 19,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 20,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 21,
      title: 'CONVERSION RATE OPTIMIZATION',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our client that result in defining prioritized cost-effective solutions that deliver a measurable Return on Investment",
      image: img,
      links: "/"
    },
    {
      id: 22,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
    {
      id: 23,
      title: 'ONLINE REPUTATION MANAGEMENT',
      date: "16 January 2024",
      desc: "We've developed an approach to shaping digital strategies for our clients that result in defining prioritized cost-effective solutions that deliver a.",
      image: img,
      links: "/"
    },
  ];